...
* JPA Hint : Hibernate (JPA 구현체)에게 제공하는 힌트



변경감지를 사용하여 변경된 객체를 감지, 업데이트 한 경우 -> 원본객체랑, 변환된 객체를 2개 관리해야 함
...
//이름이 member1인 원본과 member2인 변경된 객체 2개를 관리해야 함 -> 메모리 소모
//다시 가져오는 순간 두개를 유지하는 것
````

이때 Hibernate한테 스냅샷을 만들지 말도록 (굳이 2개 유지 x) 말해줄 수 있음 (ReadOnly할 때 등)

````java
@QueryHints(value = @QueryHint(name = "org.hibername,readOnly", value = "true"))
````

ReadOnly시에는 변경감지 체크도 안함. 읽기용 최적화



전체 어플리케이션중에서 성능 이슈는 DB쿼리장애가 대부분, 특정 부분에 집중되어 있음

-> ReadOnly 옵션 역시 진짜 중요한 부분에만 성능 테스트 이후 결정



* Lock : select 시 다른애들 건드리지 못하도록 (update 할 예정 등)

````java
@Lock(LockModeType.PESSIMISTIC_WRITE)
````

이러면 H2의 경우 쿼리에 select ... for update가 붙음 (방언에 따라 다름)

jpa가 lock을 제공하는 것



Lock은 부하가 커서 대체로 실시간 트래픽이 많은 경우에는 지양해야 함.
